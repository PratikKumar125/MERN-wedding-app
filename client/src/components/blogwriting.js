...
export default function Blogwriting() {
    const[signupdata, setsignupdata] = useState({title:"", category:"", writerofblog:"", contentofblog :""})
    const changing = (e) => {
    ...
    const submit = async()  => {
        console.log(signupdata.title);
        await axios.post("http://localhost:8000/blogpost", {title : signupdata.title, category : signupdata.category, writer : signupdata.writerofblog, bodyofblog : signupdata.contentofblog})
        console.log("blog data sent succesfully");
        setsignupdata({title:"", category:"", writerofblog:"", blogbod :""})
    }
    return (
        <div>
